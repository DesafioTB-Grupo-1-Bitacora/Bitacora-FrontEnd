...
export const getNotebooksByUser = (client) => async () => {
    try {
        const { data } = await client.get(`${PATH}/user/notebooks`);
        return data;
    } catch (error) {
        console.log("Get Notebooks By User error:", error.message);
        return { success: false }
    }
}

export const getMemoriesByNotebook = (client) => async (params) => {
    try {
        const notebook = params.queryKey[1]
        const { data } = await client.get(`${PATH}/user/notebooks/${notebook}`);
        return data;
    } catch (error) {
        console.log("Get Memories By Notebooks error:", error.message);
        return { success: false }
    }
}